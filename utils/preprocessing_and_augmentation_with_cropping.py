...
DEVICE = pick_device()
cv2.setNumThreads(0)                     # let OS schedule OpenCV
os.environ["OMP_NUM_THREADS"] = "1"      # low-priority CPU threads
...
class OptimizedHandProcessor:
    ...
    def __init__(self, target_size=(224, 224), base_margin=0.5, 
                 confidence_threshold=0.7, smooth_window=5):
        """
        Args:
...
class NSLVideoAugmenterWithCrop:
    ...
    def __init__(self, input_dataset_path: str, output_dataset_path: str, 
                 augmentation_probability: float = 0.85, apply_hand_crop: bool = True,
                 crop_target_size: tuple = (224, 224)):
        ...
        if self.apply_hand_crop:
            self.hand_processor = OptimizedHandProcessor(
                target_size=crop_target_size,
                confidence_threshold=0.2  # Low confidence for augmented videos
            )
    ...
    def _get_optimal_worker_count(self) -> int:
        ...
        try:
            ...
            # Conservative estimate: use 50% of CPU cores, max 8 workers
            optimal_workers = max(1, min(cpu_count // 2, 8))
            
            # Adjust based on available memory (assume 2GB per worker)
            memory_limited_workers = max(1, int(available_gb // 2))
            optimal_workers = min(optimal_workers, memory_limited_workers)
            
            # On MPS systems, limit to 1 worker to avoid overloading
            if torch.backends.mps.is_available():
                optimal_workers = 1
    ...
    def _process_video_batch(self, video_batch: List[Tuple[Path, Path, str]], worker_id: int) -> List[bool]:
        ...
        # Set thread limits for this worker to avoid oversubscription
        ...
        for video_path, output_path, suffix in video_batch:
            try:
                success = self.augment_and_crop_video(video_path, output_path, suffix)
    ...
    def process_entire_dataset_parallel(self, augmentations_per_video: int = 5) -> Dict[str, Dict[str, int]]:
        ...
        logger.info("="*70)
        logger.info("NSL VIDEO AUGMENTATION AND HAND CROPPING PIPELINE (PARALLEL)")
        logger.info("="*70)
