...
MODEL_PATH = './../../kaggle/training_output_videomae/checkpoints/checkpoint_epoch_main.pth'
...
NUM_FRAMES = 16   # overridden automatically if the checkpoint stores config
...
# (BatchNorm1d classifier, same as test_videomae_enhanced.py)
# ─────────────────────────────────────────────
class VideoMAEClassifier(nn.Module):
    def __init__(self, num_classes, dropout_rate=0.3,
        ...
        # Classifier WITH BatchNorm (matching trained model)
...
def predict(video_path: str,
    ...
    if isinstance(checkpoint, dict):
        if 'model_state_dict' in checkpoint:
            ...
            print(f"   Val acc : {checkpoint.get('val_accuracy', 'N/A')}")
    ...
    with torch.no_grad():
        logits = model(pixel_values)                   # shape: (1, num_classes)
