...
class VideoMAEClassifier(nn.Module):
    ...
    def __init__(self, num_classes, dropout_rate=0.3, freeze_backbone=False, 
        ...
        # Classifier WITH BatchNorm (matching your trained model)
        self.videomae.classifier = nn.Sequential(
            nn.Linear(hidden_size, hidden_size // 2),           # 0
            nn.BatchNorm1d(hidden_size // 2),                   # 1
            nn.ReLU(),                                          # 2
            nn.Dropout(dropout_rate),                           # 3
            nn.Linear(hidden_size // 2, hidden_size // 4),      # 4
            nn.BatchNorm1d(hidden_size // 4),                   # 5
            nn.ReLU(),                                          # 6
            nn.Dropout(0.2),                                    # 7
            nn.Linear(hidden_size // 4, num_classes)            # 8
        )
...
class ModelTester:
    def __init__(self, model_path, test_dir, config=None):
        ...
        self.results_dir = Path('./../../output/videomae')
...
def main():
    ...
    model_path = './../../kaggle/training_output_videomae/checkpoints/checkpoint_epoch_30.pth'  # Update path to your best model
