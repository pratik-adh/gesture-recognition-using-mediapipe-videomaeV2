...
class TestingPlotter:
    ...
    def plot_misclassification_heatmap(self, y_true, y_pred, class_names):
        ...
        total_errors = int(misclass_matrix.sum())
        error_rate   = total_errors / cm.sum() * 100

        fig_size = max(16, len(class_names) * 0.6)
        plt.figure(figsize=(fig_size, fig_size * 0.9))

        mask_zero    = misclass_matrix == 0
        mask_nonzero = ~mask_zero

        # Blank cells — neutral background
        sns.heatmap(
            misclass_matrix,
            annot=False,
            cmap=['#f7f7f7'],
        ...
        row_errors = misclass_matrix.sum(axis=1)
        ...
        logger.info(f"  Total errors: {total_errors}  ({error_rate:.1f}% error rate)")
...
def main():
    ...
    try:
        ...
        for f in [
            ...
            "plots/confusion_matrix_heatmap.png (400 DPI)",
