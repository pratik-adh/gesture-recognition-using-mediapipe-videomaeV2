#!/usr/bin/env python3
"""
Motion Transformer (MoTr) Testing Code for Mac - With Comprehensive Metrics
...
# -------------------------
# Motion Transformer Model Components
# -------------------------
class MotionExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        motion_features = []
        for t in range(T - 1):
            feat = self.motion_conv(motion[:, :, t])
...
class MotionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(T):
            frame = x[:, :, t]
            tokens = self.spatial_embed(frame)
            appearance_tokens.append(tokens)
        
        appearance_tokens = torch.cat(appearance_tokens, dim=1)
        
        B_m, C_m, T_m, H_m, W_m = motion_feat.shape
        motion_flat = motion_feat.view(B, T_m, -1)
        motion_tokens = self.motion_proj(motion_flat)
...
class TestingPlotter:
    ...
    def create_summary_report(self, metrics, config, save_path):
        ...
        summary_text = f"""
╔═══════════════════════════════════════════════════════════════════════╗
║              MOTION TRANSFORMER TESTING SUMMARY REPORT                ║
╚═══════════════════════════════════════════════════════════════════════╝
...
{'='*75}
  • Model Architecture:      Motion Transformer (MoTr)
...
class ModelTester:
    ...
    def save_results(self, metrics, class_names, y_true, y_pred, y_probs, filenames):
        ...
        with open(self.results_dir / 'classification_report.txt', 'w') as f:
            f.write("CLASSIFICATION REPORT - MOTION TRANSFORMER\n")
            f.write("=" * 80 + "\n\n")
            f.write(report)
    ...
    def print_summary(self, metrics):
        ...
        print("\n" + "="*80)
        print("TEST RESULTS SUMMARY - MOTION TRANSFORMER")
        print("="*80)
...
def main():
    ...
    model_path = './../../kaggle/training_output_motr/checkpoints/checkpoint_epoch_25.pth'
    test_dir = './../../videos_directory/npz_lightweight_videos_without_cropping_splitted_dataset/test'
    # ==================================
    
    logger.info("\n" + "="*80)
    logger.info("MOTION TRANSFORMER MODEL TESTING FOR MAC")
    logger.info("="*80)
    logger.info(f"\nModel: {model_path}")
